...
export default function Movies() {
  ...
  const getPeople = async (page) => {
    let { data } = await axios.get(`https://api.themoviedb.org/3/person/popular?api_key=d710fda48d5a99d1a685e984dd6d2608&language=en-US&page=${page}`)
    return data
  }

  const { data, isLoading, isError } = useQuery(['people', currentPage], () => getPeople(currentPage), {
  ...
  {isLoading ? <div><i className='fas fa-spinner fa-spin vh-100 d-flex justify-content-center align-items-center'></i></div> : null}
    ...
    <div className="row gap-1 d-flex justify-content-between">
      {data?.results.map((item) => <MediaItem key={item.id} mediatype={'person'} item={item} />)}
    </div>
